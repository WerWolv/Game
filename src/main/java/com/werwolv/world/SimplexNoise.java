...
public class SimplexNoise {
    ...
    private static Vector3d Vector[] = {
    ...
    private static final double F2 = 0.5*(Math.sqrt(3.0)-1.0);  //0.366025403784
    private static final double G2 = (3.0-Math.sqrt(3.0))/6.0;  //2.711324865405
    ...
    public static double noise(double xin, double yin) {
        ...
        if(t0<0) n0 = 0.0;
            ...
            t0 *= t0;
            n0 = t0 * t0 * dot(Vector[gi0], x0, y0);  // (x,y) of Vector used for 2D gradient
        }
        double t1 = 0.5 - x1*x1-y1*y1;
        if(t1<0) n1 = 0.0;
        else {
            t1 *= t1;
            n1 = t1 * t1 * dot(Vector[gi1], x1, y1);
        }
        double t2 = 0.5 - x2*x2-y2*y2;
        if(t2<0) n2 = 0.0;
        else {
            t2 *= t2;
            n2 = t2 * t2 * dot(Vector[gi2], x2, y2);
        }
